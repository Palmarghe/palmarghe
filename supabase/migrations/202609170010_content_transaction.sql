-- Save the content row and its relationships in one database transaction.
-- SECURITY INVOKER preserves authenticated table grants and RLS policies.
  ...
  if p_content_id is null then
    insert into public.content_items
      (locale,translation_group,type,status,title,slug,excerpt,body,type_data,cover_media_id,cover_url,
       author_id,featured,published_at,seo_title,seo_description,indexable,updated_at)
    values
      (v_input.locale,v_input.translation_group,v_input.type,v_input.status,v_input.title,v_input.slug,
       ...
       v_input.indexable,now()) returning id into v_id;
  else
