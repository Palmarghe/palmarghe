...
Migration `202609260015_advertising_traffic.sql` is applied in production. Studio now has **Reklam alanları** for optional Google AdSense publisher and header/article/footer slot identifiers, and **Trafik** for the last 30 days of anonymous path-level page views and unique daily visitors. Ad scripts and slots remain absent until a valid `ca-pub-…` identifier and at least one numeric slot are saved. Traffic stores no IP address, user account or user-agent; the public production endpoint returned `204` for a real write.
