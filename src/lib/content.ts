...
export interface ContentItem {
  ...
  seo_title: string | null; seo_description: string | null; translation_group: string | null; indexable: boolean;
}
...
export async function published(cookies: AstroCookies, request: Request, locale: Locale, limit = 30): Promise<ContentItem[]> {
  ...
  if (!db) return [];
  const { data, error } = await db.from('content_items').select('id,locale,title,slug,excerpt,type,body,type_data,cover_url,published_at,featured,indexable,seo_title,seo_description,translation_group').eq('locale', locale).in('status', ['published','scheduled']).lte('published_at', new Date().toISOString()).order('published_at', { ascending: false }).limit(limit);
